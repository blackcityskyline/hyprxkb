//! CapsLock monitor: polls `/sys/class/leds/*/brightness` and sends a
...
pub fn spawn(cfg: Arc<RwLock<Config>>) {
    thread::Builder::new()
        .name("capslock".into())
        .spawn(move || run(&cfg))
        ...
        if let Some(path) = &led_path {
            match read_brightness(path) {
                Some(cur) if cur != prev_brightness => {
                    ...
                    notify::send_capslock(&cfg.read().unwrap().notify, on);
                }
                None => {
